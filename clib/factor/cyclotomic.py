# Reference:
# - Factoring with Cyclotomic Polynomials (https://www.ams.org/journals/mcom/1989-52-185/S0025-5718-1989-0947467-1/S0025-5718-1989-0947467-1.pdf)

from sage.all import ZZ,Zmod,Integer,CyclotomicField
from sage.all import gcd,is_prime,factor,randrange,prod

def factor_cyclotomic_polynomial_prime_power(n,p,k,N,E,trials=-1,proof=True,verbose=False):
    """
    Factoring with k-th cyclotomic polynomial where k is a prime power.
    Generalization of <cry> in WACon 2023 Qualification

    [Explanation]
    For every positive integer n, it holds that:
    prod(phi_d(x)| (d|n)) = x^n-1

    Putting n=p^k for some prime p and integer k bigger than 1,
    phi_n(x) = (x^n-1)/(x^(n/p)-1)

    Since (q^(n/p)-1) divides (q^n-1) for every prime q,
    We could expect the existence of a multiplicative subgroup of order (q^(n/p)-1)
    which is imbedded in the finite field of order (q^n-1)

    So we pick a random irreducible polynomial f(x) of degree (n/p)
    Then we choose a polynomial ring R:=F_q[x]/f(x^p) as a domain for the algorithm
    What we, the attacker, hope here is that f(x^p) is irreducible over F_q
    to get an isomorphic ring to F_{q^n}

    Here, let g be a random polynomial of degree strictly less than (n/p) over F_q
    g(x^p) is an element of R whose order is (q^(n/p)-1),
    and we can explicitly find (q^(n/p)-1) such non-zero polynomials g
    Therefore, every element is in the subgroup of order (q^(n/p)-1) is of form g(x^p) for some g

    We suppose here that an integer E, which is a multiple of phi_n(q), is known.
    Let t be an arbitrary non-zero element of R.
    Then t^E is of form g(x^p) for some polynomial g.

    The actual computation works on R':=Z/<f(x^p),N>.
    In detail,
    1. Let t be a random element of R'.
    2. Compute t^E and let it be s.
    3. We hope the coefficient of x^i be a multiple of q, where i is relatively prime to p.
    """
    if proof:
        assert k>1
        assert p**k == n
    Z = Zmod(N)["x"]
    x = Z.gen()
    
    trial_count = 0
    while trial_count != trials:
        f = x**(n//p)+Z.random_element(n//p-1) # random monic polynomial
        if verbose:
            print(f"[i] New try with {f = }")
        R = Z.quotient(f.subs(x=x**p))
        t = R.random_element()
        s = t**E
        g = gcd(Integer(v) for i,v in enumerate(list(s)) if gcd(i,p)==1)
        g = gcd(g,N)
        if 1<g<N:
            if verbose:
                print(f"[+] Succeeded with {g=}")
            return g
        if verbose:
            if g == 1:
                print(f"[-] Failed by g=1")
            if g == N:
                print(f"[-] Failed by g=N")
        trial_count += 1
    return None

class CPFactoringRingBuilder:
    @staticmethod
    def is_primitive(g,m):
        prime_factors = [p for p,_ in factor(m-1)]
        return all(pow(g,(m-1)//p,m)!=1 for p in prime_factors)

    @staticmethod
    def generate_m(k):
        if 2%k == 1:
            return 2
        else:
            if k%2 == 1:
                k *= 2
            m = k+1
            while not is_prime(m):
                m += k
            return m

    @staticmethod
    def generate_g(m):
        prime_factors = [p for p,_ in factor(m-1)]
        g = randrange(1,m-1)
        while not all(pow(g,(m-1)//p,m)!=1 for p in prime_factors):
            g = randrange(1,m-1)
        return g

    def __init__(self,N,k):
        self.N = N
        self.k = k
        self.reset()
    
    def reset(self,N=None,k=None):
        if N==None:
            N = self.N
        if k==None:
            k = self.k
        self.N = N
        self.k = k
        self.m = None
        self.g = None
    
    def set_m(self,m,proof=True):
        if proof:
            assert is_prime(m)
        self.m = m
    
    def set_g(self,g,proof=True):
        if proof:
            assert self.m != None
            assert CPFactoringRingBuilder.is_primitive(g,self.m)
        self.g = g

    def get_m(self,k):
        if self.m:
            return self.m
        else:
            return CPFactoringRingBuilder.generate_m(k)
    
    def get_g(self,m):
        if self.g:
            return self.g
        else:
            return CPFactoringRingBuilder.generate_g(m)

    def build(self,verbose=False):
        N = self.N
        k = self.k
        m = self.get_m(k)
        g = self.get_g(m)
        if verbose:
            print("[i] Start building a ring with the following paramters:")
            print(f"- {N = }")
            print(f"- {k = }")
            print(f"- {m = }")
            print(f"- {g = }")

        F = CyclotomicField(m,"z")
        z = F.gen()
        R = F["x"]
        x = R.gen()

        h = sum(x**Integer(pow(g,i,m))for i in range(0,m-1,k))
        etas = [h(z**Integer(pow(g,j,m)))for j in range(k)]
        f_R = prod(x-eta for eta in etas)
        f = f_R.change_ring(ZZ)
        if verbose:
            print("[+] Computed period polynomial:")
            print(f"{f = }")
        
        Z = Zmod(N)
        R_m = Z.quotient(f.change_ring(Z))
        return R_m

def factor_cyclotomic_polynomial(k,N,E,trials=-1,verbose=False):
    def next_m(k,m):
        if m<2 and 2%k==1:
            return 2
        else:
            assert m%k == 1
            if k%2==0:
                k *= 2
            m += k
            while not is_prime(m):
                m += k
            return m
    
    builder = CPFactoringRingBuilder(N,k)
    m = next_m(k,1)
    
    trial_count = 0
    while trial_count != trials:
        builder.reset()
        builder.set_m(m)
        R_m = builder.build()
        
        trial_count += 1
    
